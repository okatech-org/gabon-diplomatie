...
export const Route = createFileRoute("/dashboard/")({ component: SuperadminDashboard });
		...
		<div className="flex flex-1 flex-col gap-6 p-4 pt-6 md:p-6">
			...
			<div className="grid gap-4 lg:grid-cols-7">
				<Card className="lg:col-span-3">
					<CardHeader className="flex flex-row items-center justify-between">
						...
						<Button variant="ghost" size="sm" asChild><Link to="/dashboard/audit-logs"><ArrowRight className="h-4 w-4" /></Link></Button>
					</CardHeader>
				...
				<Card className="lg:col-span-4">
					<CardHeader className="flex flex-row items-center justify-between">
						...
						<Button variant="outline" size="sm" asChild><Link to="/dashboard/requests">{t("superadmin.dashboard.viewAll")}<ArrowRight className="ml-1 h-4 w-4" /></Link></Button>
					</CardHeader>
					<CardContent>
									...
									<Table>
										...
										<TableBody>
											{stats.recentRequests.map((r: any) => (
												<TableRow key={r._id}>
													<TableCell className="font-mono text-xs"><Link to="/dashboard/requests/$requestId" params={{ requestId: r._id }} className="text-primary hover:underline">{r.reference}</Link></TableCell>
									...
									{stats.recentRequests.map((r: any) => (
										<div key={r._id} className="p-3 bg-muted/30 rounded-lg border border-border/50 space-y-2">
											<div className="flex justify-between items-start">
												<Link to="/dashboard/requests/$requestId" params={{ requestId: r._id }} className="text-primary hover:underline font-mono text-xs font-semibold">{r.reference}</Link>
			...
			<Card>
				...
				<CardContent className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
					<Button variant="outline" asChild className="justify-start h-10"><Link to="/dashboard/users"><Users className="mr-2 h-4 w-4" />{t("superadmin.nav.users")}</Link></Button>
					<Button variant="outline" asChild className="justify-start h-10"><Link to="/dashboard/orgs/new"><Plus className="mr-2 h-4 w-4" />{t("superadmin.dashboard.addOrg")}</Link></Button>
					<Button variant="outline" asChild className="justify-start h-10"><Link to="/dashboard/services"><FileText className="mr-2 h-4 w-4" />{t("superadmin.nav.services")}</Link></Button>
					<Button variant="outline" asChild className="justify-start h-10"><Link to="/dashboard/audit-logs"><ClipboardList className="mr-2 h-4 w-4" />{t("superadmin.dashboard.viewLogs")}</Link></Button>
				</CardContent>
			</Card>
		</div>
	);
